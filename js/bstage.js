document.addEventListener('DOMContentLoaded', () => {
    ...
    createTeamSheet.innerHTML = `
        <div class="bottom-sheet" style="height: 80%; background-color: #000; color: #fff;">
    ...
    addCharSheet.innerHTML = `
        <div class="bottom-sheet" style="background-color: #1c1c1e; color: #fff;">
            <div class="sheet-handle" style="background-color: #444;"></div>
    ...
    pullFriendSheet.innerHTML = `
        <div class="bottom-sheet" style="height: 70%; background-color: #1c1c1e; color: #fff;">
            <div class="sheet-handle" style="background-color: #444;"></div>
    ...
    editTeamSheet.innerHTML = `
        <div class="bottom-sheet" style="height: 80%; background-color: #1c1c1e; color: #fff;">
    ...
    chatDetailSheet.innerHTML = `
        ...
        <div class="bottom-sheet" style="height: 90%; background-color: #1c1c1e; color: #fff;">
            <div class="sheet-handle" style="background-color: #444;"></div>
            <div class="detail-sheet-content bstage-modal-content" style="padding: 0;">
                ...
                <div class="bstage-settings-list">
                    ...
                    <div class="bstage-collapse-area" id="bstage-frame-area" style="background-color: #1c1c1e;">
                        <textarea class="bstage-css-input" id="bstage-frame-css" placeholder="输入CSS代码，例如: border: 2px solid gold; border-radius: 10px;" style="background-color: #000; color: #fff; border: 1px solid #333; outline: none;"></textarea>
                        <div class="bstage-apply-btn" id="bstage-apply-frame-btn" style="background-color: #fff; color: #000;">应用</div>
                    </div>
    ...
    function openChat(member) {
        ...
        if (!member.chatHistory || !Array.isArray(member.chatHistory)) {
            member.chatHistory = [
                ...
                { isUser: false, type: 'text', text: `Hello! I'm ${member.name}.`, trans: `你好！我是${member.name}。` },
                { isUser: false, type: 'text', text: `Thanks for subscribing!`, trans: `谢谢你的订阅！` }
            ];
            saveBstageData();
        }
                ...
                if (msg.type === 'image') {
                    bubbleContent = `
                        <div class="bstage-msg-text" style="padding: 0; background: transparent;">
                            <img class="bstage-chat-image-clickable" src="${msg.imgUrl}" style="max-width: 240px; max-height: 240px; width: 100%; aspect-ratio: 1/1; border-radius: 12px; display: block; object-fit: cover; cursor: zoom-in; border: 1px solid #333;">
                        </div>
                    `;
                } else {
                ...
                if (msg.type === 'image') {
                    ...
                    if (imgNode) {
                        imgNode.addEventListener('click', (e) => {
                            e.stopPropagation();
                            if (typeof openImagePreview === 'function') openImagePreview(imgNode.src);
                            else {
        ...
        const sendMsg = () => {
            ...
            // Append User Msg
                const displayTime = msg.timestamp ? (typeof window.formatChatBubbleTime === 'function' ? window.formatChatBubbleTime(msg.timestamp) : msg.time) : (msg.time || '');
                const msgDiv = document.createElement('div');
                msgDiv.className = 'bstage-chat-msg outgoing';
                msgDiv.innerHTML = `
                    <div class="bstage-msg-status">
            ...
            member.chatHistory.push({ isUser: true, type: 'text', text: txt, time: timeString, timestamp: Date.now(), status: '未读' });
            saveBstageData();
    ...
    async function triggerChatApi(member, contentContainer) {
        ...
        msgs.slice(-fetchCount).forEach(el => {
            ...
            if (textEl) {
                ...
                text = el.querySelector('.bstage-chat-bubble').textContent;
            }
...
6. 如果是图片，请使用 "type": "image" 并在 "description" 字段中写明画面的详细提示词描述（用于 AI 画图，必须是英文提示词，逗号分隔，包含主体、环境、光影、风格等细节）。
格式示例：
[
  {"type": "text", "text": "气泡1原文", "trans": "气泡1中文翻译"},
  {"type": "image", "description": "a handsome idol, selfie, smiling, backstage, bright lighting, high quality, masterpiece"}
]
...
`;
        ...
        try {
            ...
            // Helper function to generate a 1:1 gray placeholder image with text
            function generatePlaceholderImage(description) {
                ...
                // Draw 1:1 gray background
                ctx.fillStyle = '#444444';
                ctx.fillRect(0, 0, 600, 600);
                
                // Draw text with word wrap (Larger font, centered)
                ...
                lines.forEach(l => {
                    ctx.fillText(l.trim(), 300, startY);
                    startY += lineHeight;
                });
            ...
            parsedMsgs.forEach(msgItem => {
                ...
                setTimeout(() => {
                    ...
                    if (type === 'image') {
                        // Generate a placeholder image based on description
                        const fakeImgUrl = generatePlaceholderImage(imgDesc || 'Image Preview');
                        
                        bubbleContent = `
                            <div class="bstage-msg-text" style="padding: 0; background: transparent;">
                                <img class="bstage-chat-image-clickable" src="${fakeImgUrl}" style="max-width: 240px; max-height: 240px; width: 100%; aspect-ratio: 1/1; border-radius: 12px; display: block; object-fit: cover; cursor: zoom-in; border: 1px solid #333;">
                            </div>
                        `;
                        ...
                        if (!member.chatHistory) member.chatHistory = [];
                        member.chatHistory.push({ isUser: false, type: 'image', imgUrl: fakeImgUrl, timestamp: Date.now() });
                        saveBstageData();
                    } else {
                        ...
                        if (!member.chatHistory) member.chatHistory = [];
                        member.chatHistory.push({ isUser: false, type: 'text', text: text, trans: trans, timestamp: Date.now() });
                        saveBstageData();
                    }
